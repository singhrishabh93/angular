...
angular.module('cofactrApp')
    .controller('SupportController', function($scope, $http) {
        ...
        $scope.generateTickets = function() {
            ...
            var assignees = ['John Smith', 'Sarah Johnson', 'Mike Wilson', 'Lisa Brown', 'Tom Davis'];
            
            $scope.tickets = [];
            
            for (var i = 1; i <= 20; i++) {
                var user = $scope.users[Math.floor(Math.random() * $scope.users.length)];
                var ticket = {
                    id: 'T' + (1000 + i),
                    customer: {
                        name: user.Name,
        ...
        $scope.filterTickets = function() {
            $scope.filteredTickets = $scope.tickets.filter(function(ticket) {
                ...
                var matchesStatus = !$scope.selectedStatus || ticket.status === $scope.selectedStatus;
                var matchesPriority = !$scope.selectedPriority || ticket.priority === $scope.selectedPriority;
        ...
        $scope.bulkActions = function() {
            var selectedTickets = $scope.filteredTickets.filter(function(ticket) {
